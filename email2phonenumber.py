...
def getMaskedEmailWithTwitter(phoneNumbers, victimEmail, verbose):
	...
	for phoneNumber in phoneNumbers:
		...
		if "Location" in response.headers and response.headers['Location'] == "https://twitter.com/account/password_reset_help?c=4":
			print RED + "Twitter reports MAX attemtps reached. Need to change IP. It happened while trying phone " + phoneNumber + ENDC
			continue
...
def scrapeEbay(email):
	...
	response = session.post("https://fyp.ebay.com/EnterUserInfo?ru=https%3A%2F%2Fwww.ebay.com%2F&gchru=&clientapptype=19&rmvhdr=false",
				headers = {"Cache-Control": "max-age=0",
...
def scrapePaypal(email):
	...
	if regexOutput and regexOutput.group(3):
		...
		if regexOutput.group(1):
			print GREEN + "Paypal reports that the length of the phone number (without country code) is " + len(regexOutput.group(1)) + " digits" + ENDC #TODO: remove spaces
