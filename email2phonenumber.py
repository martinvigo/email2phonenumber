    ...
    for phone_number in phone_numbers:
        ...
        if "Location" in response.headers:
            ...
            if "prevRID" in redirect_url:
                ...
                redirect_prev_rid = re.search(
                    'prevRID=([A-Z0-9]+)&', redirect_url
                )[1]
                redirect_param_jwt = re.search('paramJwt=(.*)&', redirect_url)[
                    1
                ]
                response = session.get(
                    redirect_url,
        ...
        if response.status_code >= 500:
            ...
            if not re.search(regex_email, response.text):
                ...
                continue
            masked_email = re.search(regex_email, response.text)[0]
...
def get_masked_email_twitter(phone_numbers, victim_email, verbose):
    ...
    for phone_number in phone_numbers:
        ...
        if regex_output and regex_output[1]:
            authenticity_token = regex_output[1]
        else:
            if verbose:
                logger.info(
...
def scrape_paypal(email):
    ...
    if regex_output and regex_output[1]:
        client_instance_id = regex_output[1]
    else:
        logger.info("%sPaypal did not report any digits %s", YELLOW, ENDC)
        return
    ...
    if regex_output and regex_output[3]:
        last_digits = regex_output[3]
        logger.info(
            "%sPaypal reports that the last %s digits are: %s %s",
            GREEN,
            len(last_digits),
            last_digits,
            ENDC,
...
def cache_valid_block_numbers(state, area_code):
    ...
    response = session.post(
        ...
        data=f"stateAbbr={state}&npaId={area_code}&rtCntrId=ALL&reportType=3",
        proxies=proxy,
        verify=verifyProxy,
    )
...
def set_proxy_list():
    global proxyList
    with open(args.proxies, "r") as proxy_file:
        if proxy_file.mode != 'r':
            proxy_file.close()
